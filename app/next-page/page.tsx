...
interface Photo {
  id: string;
  data: string;
  file_name: string; // Updated from fileName
  file_type: string; // Updated from fileType
  description: string;
  created_at: string; // Updated from uploadDate
}

interface PendingPhoto {
  id: string;
  data: string;
  file_name: string; // Update dari fileName
  file_type: string; // Update dari fileType
  description: string;
}
    ...
    try {
      const response = await fetch("/api/photos", {
        ...
        body: JSON.stringify({
          ...
          fileName: pendingPhoto.file_name, // ✅ SUDAH UPDATE
          fileType: pendingPhoto.file_type, // ✅ SUDAH UPDATE
              ...
              {pendingPhotos.map((pendingPhoto) => (
                ...
                >
                  <img
                    ...
                    alt={pendingPhoto.file_name}
                    className="w-full h-48 object-cover rounded-lg mb-4"
                  />

                  <div className="space-y-3">
                    <p className="text-sm text-gray-600 font-medium truncate">
                      {pendingPhoto.file_name}
                    </p>
